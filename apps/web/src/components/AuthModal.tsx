...
export default function AuthModal({ isOpen, onClose, initialTab = 'login' }: AuthModalProps) {
  ...
  const initGoogle = useCallback(() => {
    ...
    if (g && g.accounts) {
      ...
      // Render the original google button visibly
      const visibleBtn = document.getElementById('visible-google-btn')
      if (visibleBtn) {
        ...
        <div className="relative glass-panel rounded-2xl p-6 sm:p-8 shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {(activeTab === 'register' || activeTab === 'google-registration' || activeTab === 'reset-password') && <PasswordStrengthBar password={password} />}
            </div>
            )}
